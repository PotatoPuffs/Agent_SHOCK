...
# All four combinations work. The agent model and reward logic are identical
...
# def deploy_loop(model, observer, ems, dry_run: bool = False):
#     """
...
#         dry_run  : if True, suppress pyautogui clicks (EMS still fires / prints)
#     """
...
#     print(f"\n  CNN={'sim' if is_sim_observer else 'real'}  "
#           f"EMS={'sim' if hasattr(ems, 'std_scale') else 'real'}  "
#           f"dry_run={dry_run}\n")
...
#                 if not dry_run:
#                     # Click command already sent to Arduino via ems.send_action()
#                     # Arduino handles the relay timing
#                     pass
...
#             # No artificial pacing — loop runs at CNN capture speed
#             # EMS command maintains relay state between frames
...
#         help="deploy only — sim: fake observer | real: teammate CNN",
#     )
#     parser.add_argument(
#         "--ems", choices=["sim", "real"], default="sim",
#         help="deploy only — sim: print actions | real: Arduino serial",
#     )
#     parser.add_argument(
...
#         help="deploy only — suppress real pyautogui clicks",
#     )
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        # The HSV observer reads the real screen, so it must be paired with a
        # real EMS to close the loop. A sim EMS only moves a fake internal
        # cursor (and would call update_cursor on the wrong observer), so the
        # combination is rejected rather than silently running open-loop.
        # if args.cnn == "hsv" and args.ems == "sim":
            # sys.exit(
            #     "[run] --cnn hsv requires --ems real: the simulated EMS cannot "
            #     "move the real game, so the loop would be open-loop and useless.\n"
            #     "      Use '--cnn hsv --ems real', or test detection alone with "
            #     "HSVBasedObserver.debug_snapshot()."
            # )
